...
class MyDataset(Dataset):
	...
	def __init__(self, path_lip, path_aling, file_list, vid_pad, txt_pad, phase):
		self.path_aling = path_aling
		self.vid_pad = vid_pad
		self.txt_pad = txt_pad
		self.phase = phase
		with open(file_list, 'r') as f:
			self.videos = [os.path.join(path_lip, line.strip()) for line in f.readlines()]
	...
	def __getitem__(self, idx):
		...
		anno = self._load_anno(os.path.join(self.path_aling, spk, 'align', name + '.align'))
		if (self.phase == 'train'):
		...
		return {'vid': torch.FloatTensor(vid.transpose(3, 0, 1, 2)), 'txt': torch.LongTensor(anno), 'txt_len': anno_len, 'vid_len': vid_len}
